...
CKEDITOR.plugins.add("wordcount",
    {
        ...
        onLoad: function() {
            CKEDITOR.document.appendStyleSheet("assets/ckeditor/wordcount.css");
        },
        init: function(editor) {
            ...
            var bbcodePluginLoaded = typeof editor.plugins.bbcode != "undefined";
            ...
            function limitReached(editorInstance, notify) {
                ...
                if (!notify) {
                     notification = new CKEDITOR.plugins.notification(editor,
                                {
                    ...
                    // document.getElementById('errorMessage').innerText = 'Limit exceeded';
            ...
            function updateCounter(editorInstance) {
                ...
                if (config.showRemaining) {
                    if (config.maxCharCount >= 0) {
						...
						document.getElementById('errorMessage').innerText = config.maxCharCount - charCount;
						
                    } else {
                        html = html.replace("%charCount%", charCount);
						document.getElementById('errorMessage').innerText = charCount;
                    }
                ...
                function (event) {
                    ...
                    if (editor.elementMode === CKEDITOR.ELEMENT_MODE_INLINE) {
                        ...
                        if (event.data.space == "bottom") {
                            event.data.html += "<div class=\""+wordcountClass+"\"><span id=\"" +
                                counterId(event.editor) +
                                "\" class=\"cke_path_item\">&nbsp;</span></div>";
                        }
                    }
                    ...
                    if (!config.warnOnLimitOnly && (config.maxWordCount > 0 || config.maxCharCount > 0 || config.maxParagraphs > 0)) {
                        ...
                        if (notification === null) {
                            notification = new CKEDITOR.plugins.notification(event.editor,
                                {
                                    message: 'Content cannot be pasted because it is above the allowed limit',
                                    type: "warning",
                                    duration: config.pasteWarningDuration
                                });
                        }
